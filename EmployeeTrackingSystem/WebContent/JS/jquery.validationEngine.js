 ...
 * 
 * on _custom methode we are checking the invalidate cases.
 * can change the position of the message from the last line.
 * can change the properties from $.validationEngine.
 * 
 */
(function ($) {
    ...
    var methods = {
        ...
        _validateField: function (field, options, skipAjaxValidation) {
            ...
            for (var i = 0, field_errors = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "divrequired":
                        required = true;
                        errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._divrequired);
                        break;
                    case "custom":
         ...
         * @return an error string if validation failed
         */
        _divrequired: function (field, rules, i, options, condRequired) {
            var field_val   =  $('.nicEdit-main').text();
            if(!field_val) {
                    return options.allrules[rules[i]].alertText;
            }
        },
        ...
        _custom: function (field, rules, i, options) {
            ...
            if (rule["regex"]) {
                ...
                // getting  the invalidation case from the input -----------------------------------------------------        
                    ...
                    switch (rule["rule"]) {
                        ...
                        case "email":
                            if (field.val().toString().trim().length > 0) {
                                ...
                                if (split.length == 2) {
                                    var domain = split[1].trim().split('.');
                                    var domain_count = domain.length;

                                    if (domain.length == 2 && domain[0] != "") {
                                        var endings = ['org', 'com', 'net', 'edu', 'info', 'biz', 'me', 'in'];
                        ...
                        case "url":
                            if (field.val().trim().length > 0) {
                                var protocol = field.val().split("://");
                                if (protocol.length >= 2) {
                                    if (protocol[0] == "http" || protocol[0] == "https") {
                                        /*
                                         * more validation here if valid protocol
                                         */
                                    }
                                    else {
                                        return " * " + protocol[0] + " is not a valid protocol";
                                    }
                                }
                        ...
                        default :
                            if (field.val().trim().length > 0) {
                                var inputString = field.val().trim().toString().split("");
                                    ...
                                    for (var i = 0; i < inputString.length; i++) {
                                        ...
                                        if (check == null) {
                                            ...
                                            if (!inputString[i].trim()) {
                                                return "  * Remove the space near " + prvCheck;
                                            }
                                        }
                                        prvCheck = prvCheck + inputString[i];
                                    }
                 ...
                 * date before current year +100 and after current year - 100 are allowing 
                 * @param {date}
